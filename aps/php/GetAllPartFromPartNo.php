<?php
   ...
   $result = mysqli_query($mysqli, "SELECT r.PartNo, r.PImage, r.Pname, r.PCompany, r.Price, r.SubCatID, coalesce(w.Type) as WarrantyID FROM (SELECT p.PartNo, p.PImage, p.Pname, p.PCompany, p.Price, p.SubCatID, p.WarrantyID FROM Part p WHERE p.PartNo = '$partno') as r LEFT OUTER JOIN Warranty w  ON coalesce(r.WarrantyID) = coalesce(w.WarrantyID) ORDER BY PartNo");
